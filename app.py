...
def synthesize_final_memo(chunk_summaries, status_updates):
    status_updates.append("Now synthesizing final memo...")
...
"""
    ...
    try:
        ...
        status_updates.append("Final memo synthesis complete. Sending response...")
        return result["candidates"][0]["content"]["parts"][0]["text"]
    except Exception as e:
        status_updates.append(f"⚠️ Error synthesizing final memo: {e}")
...
def analyze():
    ...
    status_updates.append(f"PDF uploaded. Total pages: {len(doc)}")
    estimated_sec = max(30, len(doc) * 8)
    estimated_min = estimated_sec // 60
    status_updates.append(f"Estimated processing time: {estimated_min} min {estimated_sec % 60} sec")

    for i, page in enumerate(doc):
        status_updates.append(f"Scanning page {i+1} of {len(doc)}...")
    ...
    status_updates.append(f"Chunking complete: {len(chunks)} chunks, {chunk_size} pages per chunk")
    chunk_summaries = []

    for idx, chunk in enumerate(chunks):
        status_updates.append(f"Analyzing chunk {idx+1} of {len(chunks)}...")
